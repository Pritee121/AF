...
from datetime import datetime, timedelta
from django.db.models import JSONField  # Works with all modern DBs (Django 3.1+)
...
class Service(models.Model):
    ...
    work_days = models.ManyToManyField(WorkingTime, blank=True, related_name="services")  
    description_embedding = JSONField(null=True, blank=True)
    def save(self, *args, **kwargs):
        """Ensure total duration is calculated before saving"""
        if self.duration and self.travel_time:
            self.total_duration = self.duration + self.travel_time
        # ✅ Generate embedding if description exists and no embedding is saved yet
        if self.description and not self.description_embedding:
...
class Booking(models.Model):
    ...
    service_price = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True)  # ✅ Store price
    payment_method = models.CharField(max_length=10, choices=PAYMENT_CHOICES, default="cod")
    payment_status = models.CharField(max_length=20, default="Pending")
    status = models.CharField(max_length=10, choices=STATUS_CHOICES, default="Pending")
    ...
    created_at = models.DateTimeField(auto_now_add=True)
    is_new = models.BooleanField(default=True)  # ✅ Mark booking as new
    refund_requested = models.BooleanField(default=False)  # ✅ Add this line
    ...
    def clean(self):
        ...
        print(f"🟡 Found {overlapping_bookings.count()} overlapping booking(s):")
        for b in overlapping_bookings:
            print(f"   ❌ Booking ID {b.id} — {b.start_time} to {b.end_time} (Status: {b.status})")
